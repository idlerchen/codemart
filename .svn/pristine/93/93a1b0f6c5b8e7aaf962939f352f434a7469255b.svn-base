									...
									<div class="list-content-3dI_h">
										<header class="list-header-2Sc6-">
											<span class="search-3F9bW">
												<input class="input-DvJ_0" id="searchInput" value="">
												<span class="fa fa-search"><img src="img/searchIcon.png" style="width:15px;height:15px;margin-top:8px"></span>
											</span>
										</header>
														...
														<div class="info-body-34ARl">
															<div class="title-2q5sH">
																...
																<c:if test="${item.state==2 }">
																	<span class="status-3kUbT developing">
																</c:if>
																<c:if test="${item.state==3 }">
																	<span class="status-3kUbT finished">
																</c:if>
																${item.stateName }</span></a><span class="price-2qRFd">
															...
															<div>
																...
																<div class="label-2n6KW"><label>类型：</label><span>${item.itemtypeName }</span></div>
																<div class="label-2n6KW"><label>周期：</label><span>${item.period}</span></div>
															</div>
															<!--时间在servlet中获取到后，与当下时间确认间隔，再封装进对象里传过来渲染  -->
															<div class="time-2b2Sj"><span>${item.difftime }前发布</span>|<span>${item.applicantNum }人报名</span></div>
														</div>
													</div>
												</div>
											</div>
