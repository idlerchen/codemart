...
//	}
	...
	@Override
	public List<Apply> selectApplysByDeveloperAndItem(int developerId,int itemId) {
		String sql = "SELECT id,item_id itemId,developer_id developerId,state,note,create_time createTime"
				+ " FROM apply"
				+ " WHERE developer_id = ? and item_id = ? order by id desc";
		return this.getBeanList(sql, developerId,itemId);
	}
	...
	public List<Apply> selectApplysByState(int developerId,int state) {
		...
		String sql = "SELECT apply.id id,apply.item_id itemId,apply.developer_id developerId," + 
				...
				"FROM apply,item,apply_status,role_type,item_type " + 
				"WHERE apply.item_id = item.id AND apply.state = apply_status.id AND item.role_id = role_type.id AND item.itemtype = item_type.id " + 
				"AND apply.developer_id = ? AND apply.state = ? order by apply.id desc";
		return this.getBeanList(sql, developerId,state);
	}

	@Override
	public int updateStateByItemId(int state, int itemId) {
		String sql ="UPDATE apply SET state = ? WHERE item_id = ?";
