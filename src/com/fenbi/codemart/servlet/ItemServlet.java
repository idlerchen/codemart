...
public class ItemServlet extends BaseServlet {
	...
	public void getAllItems(HttpServletRequest request, HttpServletResponse response)
		...
		for(Item i:itemList) {
			long itemTime = i.getCreateTime().getTime();
			long nowTime = System.currentTimeMillis();
			long diff = nowTime - itemTime;
			 if(Math.floor(diff / (24 * 60 * 60 * 1000))>0 ) {
				 i.setDifftime((int)(Math.floor(diff / (24 * 60 * 60 * 1000)))+"天");
			 }else if(Math.floor(diff / (60 * 60 * 1000))>0) {
				 i.setDifftime((int)(Math.floor(diff / (60 * 60 * 1000)))+"小时");
			 }else if(Math.floor(diff / (60 * 1000))>0) {
				 i.setDifftime((int)(Math.floor(diff / (60 * 1000)))+"分钟");
			 }else {
				 i.setDifftime((int)(Math.floor(diff / (1000)))+"秒");
			 }
		}
	...
	public void getItemListByTypeAndStateAndRoleAndPageIndex(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		response.setContentType("application/json;charset=UTF-8");
		...
		List<Item> itemList = itemService.getItemListByTypeAndStateAndRoleAndPageIndex(itemtype, state, roleId, 1);
		for (Item i : itemList) {
			long itemTime = i.getCreateTime().getTime();
			long nowTime = System.currentTimeMillis();
			long diff = nowTime - itemTime;
